...
# Cargar el modelo 1
with open(model_ml_glaucoma, 'rb') as f:
    model_training = pickle.load(f)
...
# Hacer predicciones
predicciones = model_training.predict(params)
...
# Mostrar resultados
for i, (row_dict, pred, real) in enumerate(zip(params.to_dict(orient="records"), predicciones, diagnosticos_reales), 1):
    acierto = pred == real
...
print(f"Precisión del modelo en esta muestra: {aciertos / len(evaluaciones):.2%}")
